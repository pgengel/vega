USE [vega]
GO
/****** Object:  StoredProcedure [dbo].[sp_GetModel]    Script Date: 2017/05/28 8:26:50 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[sp_GetModel] 
@MakeId INT 
AS
BEGIN
  ...
  DECLARE 
	@thisProc NVARCHAR(256) = N'dbo.sp_GetModel', --used for error messages
  ...
  BEGIN TRY
    ...
    SET @errmsg = N'[sp_GetModel] - Failed to get the models';

	SELECT 
	   [Id]
      ,[MakeId]
      ,[Name]
	FROM [vega].[dbo].[tb_CarModel]
	WHERE MakeId = @MakeId
